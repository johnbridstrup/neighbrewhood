...
@brew_router.get("brewtypes", auth=JWTAuth(), response=List[BrewTypeSchema])
@paginate
def brew_types(request):
    return BrewType.objects.all()

@brew_router.get("qualities", auth=JWTAuth(), response=List[QualitySchema])
@paginate
def qualities(request):
    return Quality.objects.all()

@brew_router.get("brews", auth=JWTAuth(), response=List[BrewResponseSchema])
@paginate
def brews(request):
    return Brew.objects.all()

@brew_router.get("brews/{id}", auth=JWTAuth(), response={200: BrewResponseSchema, codes_4xx: DefaultError})
...
@brew_router.post("createBrew", auth=JWTAuth(), response=BrewResponseSchema)
